...
static void rewrite_code(void)
{
    ...
    unsigned long inode;
    char pathname[128] = ""; // 預設空字串

    while (fgets(buf, sizeof(buf), mem_info) != NULL) {
        // Format : from-to, perms, offset, dev_major, dev_minor, inode (pathname)
        int fields = sscanf(buf, "%" PRIxPTR "-%" PRIxPTR " %4s %lx %x:%x %lu %s", &from, &to, perms, &offset, &dev_major, &dev_minor, &inode, pathname);
        // do not touch stack, vdso, and vsyscall
        if (((strcmp(pathname, "[stack]") != 0 ) && (strcmp(pathname, "[vdso]") != 0 ) && (strcmp(pathname, "[vsyscall]") != 0 ))) {
...
static void load_hook_lib(void)
{
    ...
    const char *lib_hook = getenv("LIBZPHOOK");
    if (lib_hook == NULL) {
        return;
    }

    handle = dlmopen(LM_ID_NEWLM, lib_hook, RTLD_NOW | RTLD_LOCAL);
    ...
    __hook_init hook_init = (__hook_init) dlsym(handle, "__hook_init");

    // Check for errors after calling dlsym
    const char *dlsym_error = dlerror();
    if (dlsym_error != NULL) {
        fprintf(stderr, "Symbol lookup error for '__hook_init': %s\n", dlsym_error);
        dlclose(handle); // Close the handle on error
        exit(1);
    }

    // Also check if the symbol was found but is NULL
    if (hook_init == NULL) {
        fprintf(stderr, "Error: Found '__hook_init' symbol in '%s', but it is NULL.\n", getenv("LIBZPHOOK"));
        dlclose(handle);
        exit(1);
    }
